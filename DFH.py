...
class DiffieHellman:
    ...
    def get_prime(group):
        """
		Accepts group number, returns a Prime number.
		"""
    ...
    def gen_public_key(self):
        """
		Generates public key Х = g^x % p.
		"""
        return pow(self.generator, self.privateKey, self.prime)

    def check_public_key(self, received_key):
        # Проверка полученного ключа на соответсвие стандарту NIST SP800-56
        # 2 <= g^b <= p-2 и  (g^bq)=1, q=(p-1)/2

        if 2 <= received_key <= self.prime - 2:
            if pow(received_key, (self.prime - 1) // 2, self.prime) == 1:
                return True
        return False

    def gen_key(self, received_key):
        """
		Derive the shared secret, then hash it to obtain the shared key.
		"""

        if self.check_public_key(received_key):
            self.Secret = pow(received_key, self.privateKey, self.prime)
        else:
